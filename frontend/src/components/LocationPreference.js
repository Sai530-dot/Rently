...
const LocationPreference = ({ onNext, userPreferences, setUserPreferences }) => {
  ...
  const handleContinue = () => {
    // Allow continuing with either selected location or manual input
    if (selectedLocation) {
      setUserPreferences({ ...userPreferences, location: selectedLocation });
      onNext();
    } else if (inputValue.trim()) {
      // Create a basic location object from manual input
      const manualLocation = {
        formatted: inputValue.trim(),
        city: inputValue.trim(),
        address_line1: inputValue.trim()
      };
      setUserPreferences({ ...userPreferences, location: manualLocation });
      onNext();
    }
  };
      ...
      <div className="preferences-form">
        ...
        {!selectedLocation && inputValue.trim() && (
          <p style={{ 
            ...
            💡 Tip: Select from suggestions or press Continue to use "{inputValue}"
          </p>
        )}
      </div>
